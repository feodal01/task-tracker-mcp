...
async def test_close_task():
    ...
    close_result = await mcp_service.update_status(
        task_id=task_id, status="done", reason="Completed"
    )
...
async def test_list_tasks():
    await mcp_service.create_task(
        description="Task 1", dod="DoD 1", deadline=None, assignee=None, parent_id=None
    )
    await mcp_service.create_task(
        description="Task 2", dod="DoD 2", deadline=None, assignee=None, parent_id=None
    )
